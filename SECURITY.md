...
- **Registry-signature verification for the pnpm lockfile** remains tracked.
  npm's current `npm audit signatures` implementation expects an npm lockfile;
  generating a transient `package-lock.json` in this pnpm workspace forces npm
  to re-resolve a second dependency graph that can diverge from the committed
  `pnpm-lock.yaml`. Daloy therefore does not run that gate until there is a
  pnpm-native signature/provenance verifier, or npm can verify signatures from
  the installed pnpm tree without re-resolution. Existing release protection is
  still provided by `pnpm install --frozen-lockfile --ignore-scripts`,
  `pnpm verify:lockfile`, sha512 lockfile integrity, `verify-store-integrity`,
  SBOM verification, zero runtime dependencies, and npm trusted publishing with
  `--provenance`.
...
- An attacker who compromises a Daloy *dev* dependency, gets past the 24 h
  ...
  the residual risk is non-zero. Registry-signature verification remains
  tracked as a pnpm-tooling gap; the release workflow does not generate a
  transient npm lockfile because that would verify a dependency graph different
  from the committed `pnpm-lock.yaml`. SLSA build-level-3 attestations remain
  tracked (the CycloneDX 1.5 + SPDX 2.3
