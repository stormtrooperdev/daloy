  ...
  return (
    <>
      ...
      <p>
        ...
        client IP / time-claim / secret comparison, the{" "}
        <code>__Secure-</code> cookie without TLS refuse-to-boot, the{" "}
      ...
      <p>
        ...
        well-documented &quot;developer flipped the flag off while debugging
        and shipped to production&quot; footgun by forcing an explicit
        two-step opt-in:
      </p>
      <CodeBlock
        language="ts"
...
new App({
  ...
  acknowledgeInsecureDefaults: true,
      ...
      />
      <p>
        Any time the flag is off, a once-per-process <code>error</code>{" "}
        log is emitted with{" "}
        <code>{`event: "secure_defaults.disabled"`}</code> enumerating every
        default it disabled — so the blast radius is loud at boot even when
        the option was set deep in shared configuration:
      </p>
      <ul>
        <li>auto <code>secureHeaders</code> install</li>
        <li>cross-origin guard for state-changing requests</li>
        <li>crash-on-unhandled-rejection (production)</li>
        <li>first-request <code>X-Forwarded-*</code> / <code>trustProxy</code> guard</li>
        <li><code>session()</code> + state-changing route requires <code>csrf()</code> boot guard</li>
        <li>weak session secret refuse-to-boot</li>
        <li><code>cors({"{ origin: '*' }"})</code> refuse-to-boot</li>
      ...
      <p>
        ...
        remain available without the production refusal — prefer those when
        you only need to disable one default rather than the whole surface.
        Tests can reset the audit-log latch via the exported{" "}
      ...
      <p>
        <code>createJwtSigner()</code> and <code>createJwtVerifier()</code>{" "}
        now refuse <code>Uint8Array</code> HS-shaped secrets shorter than{" "}
...
createJwtSigner({ alg: "HS256", key, maxLifetimeSeconds: 60 });`}
      ...
      <p>
        ...
        clickjacking: the <code>X-Frame-Options</code> header (legacy
        browsers) and a CSP <code>frame-ancestors</code> directive (modern
        spec). The helper now refuses to construct when{" "}
        <em>both</em> are disabled simultaneously — that combination silently
        re-opens the clickjacking surface the helper exists to close:
      </p>
      <CodeBlock
        language="ts"
        code={`// ❌ refuses
...
secureHeaders({
  ...
  contentSecurityPolicy: {
    ...
    "frame-ancestors": ["'none'"],
      ...
      />
      <p>
        If you only want to disable one of the two defenses, keep the other
        one on — the helper&apos;s defaults already wire both layers, so the
        common case (no options passed) needs no changes.
      </p>
      ...
      <ul>
        <li>
          ...
          <code>Settings → Authentication security → Require two-factor authentication</code>{" "}
          is enforced on the <code>@daloyjs</code> org; every account with
          write access must have a hardware-backed factor (passkey or security
          key — TOTP-only accounts are off-boarded).
        </li>
        <li>
          <strong>npm registry level:</strong> <code>npm access 2fa-required</code>{" "}
          is set on <code>@daloyjs/core</code> and <code>create-daloy</code>;
          OIDC trusted publishing from the protected{" "}
          <code>npm-publish</code> environment means publishes themselves
          carry no long-lived token, but every maintainer who can approve the
          environment still needs hardware-backed 2FA on the registry account.
        </li>
        <li>
          <strong>Off-boarding:</strong> when a maintainer leaves rotation,
          their org membership, publish grants, and granular tokens are
          revoked in the same change.
        </li>
        <li>
          ...
          contributor who approved the <code>npm-publish</code> Environment
          for that release has 2FA enabled at both levels (the Wave 8
          mandatory-2FA audit gate).
        </li>
      </ul>
      ...
      <p>
        ...
        comparison; the <code>__Secure-</code> cookie without TLS
        refuse-to-boot guard; the <code>daloy doctor --audit-secrets</code>{" "}
        subcommand; the zero-external-runtime-dependency governance CI grep
        gate; and the timing-safe-comparison CI grep gate. Together these
        items remove the last &quot;developer remembered to do X but not
        Y&quot; failure modes by making the framework&apos;s security surface
